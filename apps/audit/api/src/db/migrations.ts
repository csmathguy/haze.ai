...
export async function applyPendingAuditMigrations(databaseUrl: string): Promise<number> {
...
export async function ensureAuditDatabaseReady(databaseUrl: string): Promise<void> {
  if (preparedDatabases.has(databaseUrl)) {
    return;
  }

  await applyPendingAuditMigrations(databaseUrl);
  preparedDatabases.add(databaseUrl);
}
